#!/usr/bin/env bash
# M0 headless gate: authority provenance + FPC harness + admitted value/reader mechanics.
set -euo pipefail
...
# M0.3 value layer: mechanism only.
...
grep -Eq '^VALUES: pass=[0-9]+ fail=0$' <<<"$value_output"
...
grep -Eq '^READER: pass=[0-9]+ fail=0$' <<<"$reader_output"
