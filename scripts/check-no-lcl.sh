...
grep -nHEn "^(uses|Uses)[^;]*(Forms|Controls|Lcl)" src/*.pas 2>/dev/null > "$tmp" || true
...
grep -nHEn "\bVariant\b|\bTObject\b|\bTVarRec\b" src/*.pas 2>/dev/null \
...
grep -nHEn "\bT?(Form|Control)\b" src/*.pas 2>/dev/null > "$tmp" || true
